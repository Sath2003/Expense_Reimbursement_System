...
class ImprovedReceiptExtractor:
    ...
    AMOUNT_PATTERNS = [
        # Most specific: Total/Final patterns with rupee symbol (highest priority)
        ...
        # Bare rupee symbol - clear indicator
        (r'₹\s*([\d,]+\.?\d*)', 'rupee_symbol'),
        (r'Rs\.?\s*([\d,]+\.?\d*)', 'rs_abbrev'),
        (r'INR\s*([\d,]+\.?\d*)', 'inr_code'),
        
        # Text-based patterns with keywords (more comprehensive)
        (r'(?:total|amount|price|cost|due|payable|fare|paid|bill|sum|charge)\s+(?:is|:|=)?\s*(?:Rs|rupees?|INR)\s*[\.]?\s*([\d,]+\.?\d*)', 'text_total'),
        (r'rupees?\s*([\d,]+\.?\d*)', 'rupees_text'),
        
        # Generic patterns with keywords
        (r'(?:total|amount|price|cost|bill|payable|fare|ticket|sum|charge|balance|due)[\s:=]*([₹Rs]?\s*[\d,]+\.?\d*)', 'generic_total'),
        
        # Standalone currency indicators
        (r'(?:Rs|INR|₹)\s*([\d,]+\.?\d*)', 'currency_context'),
        
        # Table-style amounts (common in receipts)
        (r'([\d,]+\.?\d*)\s*(?:Rs|INR|₹)', 'table_amount'),
        
        # Decimal patterns with commas (Indian format)
        (r'([\d]{1,2},[\d]{2},[\d]{3}\.?\d*)', 'indian_format'),
        (r'([\d]{1,3},[\d]{3}\.?\d*)', 'western_format'),
        
        # Amount in parentheses or after currency
        (r'[\(\[]\s*([₹Rs]?\s*[\d,]+\.?\d*)\s*[\)\]]', 'parentheses_amount'),
        
        # Common receipt patterns
        (r'(?:amt|amount)\s*[:\-]?\s*([₹Rs]?\s*[\d,]+\.?\d*)', 'amt_pattern'),
        (r'(?:pay|paid)\s*[:\-]?\s*([₹Rs]?\s*[\d,]+\.?\d*)', 'pay_pattern'),
        
        # Numbers with 2 decimal places (likely amounts)
        (r'([\d,]+\.\d{2})\s*(?:Rs|INR|₹)?', 'decimal_amount'),
        
        # Large numbers (likely amounts vs small numbers like dates)
        (r'([\d]{3,6}(?:\.\d{2})?)\s*(?:Rs|INR|₹)?', 'large_number'),
        
        # NOTE: Fallback pattern removed to avoid matching years or unrelated numbers
        # Only extract amounts with clear context (rupee symbol, amount keywords, etc)
    ]
    ...
    def _normalize_amount(amount_str: str) -> Optional[float]:
        ...
        Filters out year-like patterns (19XX, 20XX).
        """
        if not amount_str:
            return None
...
def extract_from_image(file_path: str) -> Tuple[Optional[float], str, str]:
    ...
    def extract_from_word(file_path: str) -> Tuple[Optional[float], str, str]:
        ...
        try:
            ...
            # Extract text from tables with better processing
            ...
            print(f"[DEBUG] Word document extracted text: {full_text[:800]}...")
            
            # Search for amounts using patterns
            for pattern, pattern_name in ImprovedReceiptExtractor.AMOUNT_PATTERNS:
                matches = re.finditer(pattern, full_text, re.IGNORECASE)
                for match in matches:
                    amount_str = match.group(1)
